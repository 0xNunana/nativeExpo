import { StyleSheet, FlatList, Text, View, TextInput, TouchableOpacity, Alert, Switch } from 'react-native'
...
const ColorModal = ({ navigation }) => {
    ...
    const handleSubmit = useCallback(() => {
        if (!typed) {
            ...
            Alert.alert("select a minimum of three colors")
    ...
    const handleValueChange = useCallback((value, color) => {
        if (value === true) {
            ...
            setSelected(colors => colors.filter(selectedColor => color.colorName !== selectedColor.colorName))
        }
    }, [selected, setSelected])

    return (
        <View style={{ padding: 10, backgroundColor: 'orange', flex: 1 }}>
            <Text style={{ fontSize: 15, paddingVertical: 3 }}>Name of the palette</Text>
            <TextInput value={typed} onChangeText={setTyped} placeholder='Palette Name' style={{ height: 50, borderRadius: 5, borderColor: 'blue', borderWidth: 1, backgroundColor: 'white', padding: 5, marginBottom: 50 }} />

            <FlatList
                data={COLORS}
                keyExtractor={item => item.colorName}
                renderItem={({ item }) => (
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: 'white' }}>
                        <Text>{item.colorName}</Text>
                        <Switch value={
                            //if selected is truthy(!!) by matching color.colorName to the item.colorName
                            !!selected.find(color => color.colorName === item.colorName)} onValueChange={newValue => handleValueChange(newValue, item)} />
                    </View>
                )}
            />


            <TouchableOpacity onPress={handleSubmit} style={{ height: 60, backgroundColor: 'teal', borderRadius: 5, justifyContent: 'center', alignItems: 'center', marginTop: 10 }}>
